...
string UploadThread::qstr2str(const QString qstr)
{
    QByteArray cdata = qstr.toLocal8Bit();
    return std::string(cdata);
}

void UploadThread::setFileAbsPath(QString str)
{
...
void UploadThread::run()
{
    ...
    if(fp==NULL){
        ...
        tcpClient->send_file(name,2,nullptr,0,g_fileSize);
        //qDebug()<<"发送完成";
    }
    fclose(fp);
    emit isDone();
